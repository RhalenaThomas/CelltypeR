...
#' \\dontrun{
#' @importFrom dplyr filter select group_by select mutate
#' \\dontrun{
#' 
...
#' \\dontrun{
#' now accounts for negative values that become NaN
#' \\dontrun{
#' }
#' \\dontrun{
#' }
...
find_correlation <- function(test,
  ...
  #cat("the test vector is",length(testc),"\n")
  #cat("the reference vector is",length(refc),"\n")
  ...
  for (i in 1:nrow(test)) {
    ...
    result <- c(result,
                rownames(test)[i], #col 1: cell sample index
                corr_ls[top[1]], #col 2: 1st correlation
                rownames(reference)[top[1]], #col 3: 1st best cell type
                corr_ls[top[2]], #col 4: 2nd correlation
                rownames(reference)[top[2]], #col 5: 2nd best cell type
                ifelse(corr_ls[top[1]] < min_corr, "unassigned",
                       ifelse(corr_ls[top[1]] - corr_ls[top[2]] < min_diff,
                              paste(rownames(reference)[top[1]],
                                    rownames(reference)[top[2]], sep = "-"),
                              rownames(reference)[top[1]]))) #col 6: assigned cell type
...
plot_corr <- function(df, threshold = 0, min_cells = 100) {
  ...
  plot5 <-
    ...
    geom_line(show.legend = F, linewidth = 0.1, color = "black") +
    geom_point()+
  ...
  if (length(unique(df.melt.double$cell.label)) > 1) {
    plot6 <- ggplot(df.melt.double, aes(x = variable, y = value, colour = variable, group = X)) +
      geom_line(show.legend = FALSE, linewidth = 0.1, color = "black") +
      geom_point() +
      scale_color_manual(values = c("#4E84C4", "#52854C","purple","orange")) +
      #ylim(-0.15, 0.8) +
...
# this will make a dataframe - if we add in the query and reference it makes a seurat object
  ...
  # seu.q <- AddMetaData(seu.q, metadata = pred) # adds everyting
  seu.q <- AddMetaData(seu.q, metadata = pred$predicted.id, col.name = pred_label)
  return(seu.q)
}
